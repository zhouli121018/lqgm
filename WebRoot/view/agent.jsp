...
<body onload="getAccount(1,0,'<%=basePath%>',${sessionScope.manager.powerId==1})">
    ...
    <div id="agent" class="active">
            ...
            <div class="panel-body">
                <div class="clear time">
                    <ul class="nav nav-pills pull-right">
                    	<li  class="active"><a href="#" onclick="getAccount(1,0,'<%=basePath%>',${sessionScope.manager.powerId==1})">全部充值</a><li>
                        ...
                        <li><a href="#" onclick="getAccount(1,5,'<%=basePath%>',${sessionScope.manager.powerId==1})">今天充值</a><li>
                    </ul>
                </div>
                ...
                <ul id="agent-pages"></ul>
                    ...
                    <!--<div class="row">-->
                        ...
                        <!--<div class="col-xs-8 col-sm-9 col-md-10">三级代理总数量：<span>3</span></div>-->
                    <!--</div>-->
        ...
        <div id="agentDetail">
            <form>
                ...
                <input type="button"  class="btn  btn-success sure" onclick="saveManagerInfo(this)" value="修 改"/>
                <input type="button" class="btn  btn-danger cancel" value="取 消"/>
            </form>
        </div>
         <div id="charge">
			...
			$.ajax({	
				   ...
				   success: function data(data){
							...
							if(param.status == "0"){
								  ...
								  if(param.managerId){
									  var html='';
// 									  html=`<tr><td>"+name+"</td><td>"+param.managerId+"</td><td>"+telephone+"</td><td>"+weixin+"</td>"+"<td>"+qq+"</td><td>"+inviteCode+"</td><td>"+uuid+"</td><td>"+"</td><td>0</td><td>正常</td><td>"+rootManager==1?'是':'否'+"</td><td>"+powerId==5?'总代理':(powerId==4?'一级代理':(powerId==3?'二级代理':(powerId==2?'三级代理':'管理员')))+"</td>"+
// 									  "<td class='adminEdit'><a href='"+param.managerId+"' class='btn btn-sm btn-warning'>修改</a> <a href='"+param.managerId+"' class='btn btn-sm btn-success charge'>充房卡</a> <a class='btn btn-sm btn-danger delete'  href='"+param.managerId+"'>删除</a>  <a href='"+param.managerId+"' class='btn btn-sm btn-primary resetPwd'>密码重置</a></td></tr>`;
									 ...
									 var powerId = powerId==5?'总代理':(powerId==4?'一级代理':(powerId==3?'二级代理':(powerId==2?'三级代理':'管理员')));
									  html="<tr>"+
									  "<td>"+name+"</td>"+
